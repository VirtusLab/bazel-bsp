  ...
  private fun compareWorkspaceTargetsResults(): BazelBspTestScenarioStep =
    BazelBspTestScenarioStep(
      ...
      testClient.test(5.minutes) { session, _ ->
        // Sync can be slow in CI/sandbox (Bazel 7); allow time for first sync
        var targetsResult = session.server.workspaceBuildTargets().await()
        if (targetsResult.targets.isEmpty()) {
          kotlinx.coroutines.delay(5.seconds)
          targetsResult = session.server.workspaceBuildTargets().await()
        }

        targetsResult.targets.size shouldBe 4
        val innerRepo = "inner$bzlmodRepoNameSeparator"
        targetsResult.targets.map { Label.parse(it.id.uri) } shouldContainExactlyInAnyOrder
          listOf(
            Label.parse("@@$innerRepo//:lib_inner"),
            Label.parse("@@$innerRepo//:bin_inner"),
  ...
  private fun compareWorkspaceRepoMappingResults(): BazelBspTestScenarioStep =
    BazelBspTestScenarioStep(
      ...
      testClient.test(5.minutes) { session, _ ->
        ...
        val innerCanonical = "inner$bzlmodRepoNameSeparator"
        val rulesJavaCanonical = "rules_java$bzlmodRepoNameSeparator"
        val rulesPythonCanonical = "rules_python$bzlmodRepoNameSeparator"
        repoMapping.apparentRepoNameToCanonicalName shouldBe
          mapOf(
            ...
            "rules_java" to rulesJavaCanonical,
            "rules_python" to rulesPythonCanonical,
            "bazel_tools" to "bazel_tools",
            "outer" to "",
            "inner" to innerCanonical,
          )
        ...
        canonicalMapping.getValue(rulesJavaCanonical).shouldEndWith("/external/$rulesJavaCanonical/")
        canonicalMapping[innerCanonical] shouldBe "file://$workspaceDir/inner/"
